...
export default function ChatbotPluginsClientPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const chatbotPluginIds = new Set(chatbotData.plugins.map(p => p.pluginId));
        const filteredPlugins = pluginsData.filter(plugin => !chatbotPluginIds.has(plugin.id));
  ...
  const handleRemovePlugin = async (pluginId: number) => {
    ...
    try {
      ...
      setChatbot(prevChatbot => {
        ...
        return {
          ...prevChatbot,
          plugins: prevChatbot.plugins.filter(plugin => plugin.pluginId !== pluginId)
        };
      });
      
      // Add the removed plugin back to available plugins
      const removedPlugin = chatbot?.plugins.find(plugin => plugin.pluginId === pluginId)?.plugin;
      if (removedPlugin) {
  ...
  const handleAddPlugins = async () => {
    ...
    try {
      ...
      setAvailablePlugins(prev => prev.filter(plugin => !addedPluginIds.has(plugin.id)));
  ...
  const handlePluginSelectionChange = (pluginId: number, isChecked: boolean) => {
    if (isChecked) {
    ...
    return (
      <div className="container py-8">
        <Alert variant="destructive" className="mb-4">
          ...
          <AlertDescription>Chatbot not found or you don't have access to it.</AlertDescription>
        </Alert>
        <Button onClick={() => router.push('/chatbot')}>Back to Chatbots</Button>
      </div>
    );
  }
    ...
    <div className="container py-8">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-muted-foreground">{chatbot.description || "Manage the plugins for this chatbot"}</p>
        </div>
                    ...
                    <div key={plugin.id} className="flex items-start space-x-2">
                      <Checkbox 
                        ...
                        onCheckedChange={(checked) => 
                          handlePluginSelectionChange(plugin.id, checked === true)
                        }
                      />
      ...
      {chatbot.plugins.length === 0 ? (
        <div className="bg-muted p-8 rounded-lg text-center">
          ...
          <p className="text-muted-foreground mb-4">Add plugins to enhance your chatbot's capabilities.</p>
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            <DialogTrigger asChild>
              <Button disabled={availablePlugins.length === 0}>
                <Plus className="mr-2 h-4 w-4" />
          ...
          {chatbot.plugins.map((chatbotPlugin) => (
            <Card key={chatbotPlugin.pluginId}>
              <CardHeader className="pb-2">
                ...
                <CardDescription>Status: {chatbotPlugin.enabled ? 'Enabled' : 'Disabled'}</CardDescription>
              </CardHeader>
